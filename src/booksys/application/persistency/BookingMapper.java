...
public class BookingMapper
{
  ...
  public Vector getBookings(Date date)
  {
	...
	= Database.getInstance().getConnection().createStatement() ;
      ResultSet rset
	= stmt.executeQuery("SELECT * FROM booksys.Reservation WHERE date='"
			    + date + "'") ;
      while (rset.next()) {
	...
	v.add(r) ;
      ...
      rset.close() ;
      rset = stmt.executeQuery("SELECT * FROM booksys.WalkIn WHERE date='"
			       + date + "'") ;
      while (rset.next()) {
	int oid = rset.getInt(1) ;
	int covers = rset.getInt(2) ;
						 ...
						 Time arrivalTime)
  {
    ...
    performUpdate("INSERT INTO booksys.Reservation " + "VALUES ('"
		  + oid + "', '"
		  + covers + "', '"
		  + date + "', '"
		  + time + "', '"
  ...
  {
    int oid = Database.getInstance().getId() ;
    performUpdate("INSERT INTO booksys.WalkIn " + "VALUES ('"
		  + oid + "', '"
		  + covers + "', '"
		  + date + "', '"
		  + time + "', '"
  ...
  public void deleteBooking(Booking b)
  {
    ...
    performUpdate("DELETE FROM booksys." + table + " WHERE oid = '"
