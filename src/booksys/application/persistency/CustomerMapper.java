...
public class CustomerMapper
{
  ...
  private PersistentCustomer getFromCacheByDetails(String name, String phone)
  {
    ...
    Enumeration num = cache.elements() ;
    while (c == null && num.hasMoreElements()) {
      PersistentCustomer tmp = (PersistentCustomer) num.nextElement() ;
      if (name.equals(tmp.getName()) && phone.equals(tmp.getPhoneNumber())) {
	c = tmp ;
      }
    }
    return c ;//name , phonenumber , oid ∏Æ≈œ
  }
  ...
  public PersistentCustomer getCustomer(String n, String p)//name , phone number
  {
    PersistentCustomer c = getFromCacheByDetails(n, p) ;
    if (c == null) {
      c = getCustomer("SELECT * FROM booksys.Customer WHERE name = '" + n
		      + "' AND phoneNumber = '" + p + "'") ;
      if (c == null) {
  ...
  PersistentCustomer getCustomerForOid(int oid)
  {
    ...
    if (c == null) {
      c = getCustomer("SELECT * FROM booksys.Customer WHERE oid ='" + oid + "'") ;
      if (c != null) {
	addToCache(c) ;
      }
    }
  ...
  PersistentCustomer createCustomer(String name, String phone)
  {
	...
	int updateCount
	  = stmt.executeUpdate("INSERT INTO booksys.Customer (name, phoneNumber)" +
			       "VALUES ('" + name + "', '" + phone + "')") ;
	stmt.close() ;
      }
