...
public class TableMapper
{
  ...
  private PersistentTable getFromCacheByNumber(int tno)
  {
    ...
    while (t == null & num.hasMoreElements()) {
      PersistentTable tmp = (PersistentTable) num.nextElement() ;
  ...
  public PersistentTable getTable(int tno)
  {
    ...
    if (t == null) {
      t = getTable("SELECT * FROM booksys.Table WHERE number='" + tno + "'") ;
      if (t != null) {
	addToCache(t) ;
      }
    }
  ...
  PersistentTable getTableForOid(int oid)
  {
    ...
    if (t == null) {
      t = getTable("SELECT * FROM booksys.Table WHERE oid ='" + oid + "'") ;
      if (t != null) {
	addToCache(t) ;
      }
    }
  ...
  public Vector getTableNumbers()
  {
	...
	= Database.getInstance().getConnection().createStatement() ;
      ResultSet rset
	= stmt.executeQuery("SELECT * FROM booksys.Table ORDER BY number") ;
      while (rset.next()) {
